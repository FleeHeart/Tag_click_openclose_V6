...
class CTkMultiButtonRandomClickPanel(ctk.CTkFrame):
    def __init__(self, master, auto_click_manager, on_back_callback):
    ...
    def _init_ui(self):
        ...
        self.refresh_tabs_button.pack(pady=10)
        ...
        # 目标XPath输入区域
        xpath_label = ctk.CTkLabel(self.target_frame, text="目标XPath (点击'+添加'按钮添加更多):")
        xpath_label.pack(anchor="w", padx=10, pady=5)
        
        # 初始XPath输入框
        self.xpath_entries_frame = ctk.CTkFrame(self.target_frame, fg_color="transparent")
        self.xpath_entries_frame.pack(fill="x", padx=10, pady=5)
        
        # 添加第一个XPath输入框
        self.add_xpath_entry()
        
        # 添加XPath按钮
        add_xpath_button = ctk.CTkButton(
            self.target_frame,
            text="➕ 添加XPath",
            font=ctk.CTkFont(size=12),
            command=self.add_xpath_entry
        )
        add_xpath_button.pack(anchor="w", padx=10, pady=5)
        ...
        self.interval_slider.set(2)  # 设置默认值为2秒
        
        self.interval_value_label = ctk.CTkLabel(interval_frame, text="2秒")
        ...
        self.test_button = ctk.CTkButton(
            ...
            text="🔍 测试随机点击",
            font=ctk.CTkFont(size=12),
            command=self.test_random_click
        )
        ...
        # 默认时间间隔设置为2秒
        self.auto_click_manager.delivery_interval = 2
    ...
    def get_random_interval(self):
        ...
        try:
            ...
            # 如果输入无效，返回默认值
            return self.auto_click_manager.delivery_interval if self.auto_click_manager else 2
        ...
        if 0 <= index < len(self.browser_tabs):
            try:
                if self.auto_click_manager.switch_to_tab_by_index(index):
                    ...
                    # 设置目标标签页URL
                    self.auto_click_manager.set_target_tab_url(current_url)
            except Exception as e:
                logger.error(f"切换标签页失败: {str(e)}")
    
    def add_xpath_entry(self):
        """添加一个XPath输入框"""
        entry_frame = ctk.CTkFrame(self.xpath_entries_frame, fg_color="transparent")
        entry_frame.pack(fill="x", pady=2)
        
        xpath_entry = ctk.CTkEntry(entry_frame, placeholder_text="输入XPath，例如: //button[@id='submit']")
        xpath_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))
        ...
        remove_button = ctk.CTkButton(
            ...
            text="➖",
            width=30,
            command=lambda frame=entry_frame, entry=xpath_entry: self.remove_xpath_entry(frame, entry)
        )
        remove_button.pack(side="left")
        
        self.xpath_entries.append(xpath_entry)
    
    def remove_xpath_entry(self, frame, entry):
        """删除一个XPath输入框"""
        if len(self.xpath_entries) > 1:  # 确保至少保留一个输入框
            frame.destroy()
            self.xpath_entries.remove(entry)
        else:
            # 显示提示消息
            from CTkMessagebox import CTkMessagebox
            CTkMessagebox(
                title="无法删除",
                message="至少需要保留一个XPath输入框",
                icon="warning"
            )
    ...
    def toggle_auto_click(self):
        ...
        # 获取所有XPath
        xpaths = [entry.get() for entry in self.xpath_entries if entry.get()]
        
        if not xpaths:
            from CTkMessagebox import CTkMessagebox
            CTkMessagebox(
                title="错误",
                message="至少需要设置一个XPath",
                icon="error"
            )
            return
        ...
        self.auto_click_manager.set_target_xpaths(xpaths)  # 注意这里使用新方法
    ...
    def test_random_click(self):
        ...
        target_url = self.url_entry.get()
        self.auto_click_manager.set_target_url(target_url)
        self.auto_click_manager.set_target_xpaths(xpaths)
        
        # 执行随机点击
        try:
            result = self.auto_click_manager.perform_random_click()
            if result:
                logger.info("测试随机点击成功")
            else:
                logger.warning("测试随机点击失败")
        except Exception as e:
            logger.error(f"测试随机点击时发生错误: {str(e)}")

    # 添加停止方法
    def stop_auto_click(self):
        if self.auto_click_manager.is_auto_delivery_running:
